...
public class EntityShader extends ShaderProgram {
	...
	private static final String VERTEX_FILE = "/shaders/entityvertexShader.glsl";
	private static final String FRAGMENT_FILE = "/shaders/entityfragmentShader.glsl";
	
	private int location_fixPositionMatrix;
	...
	private int location_boneTransforms[];
	private int location_bindShapeMatrix;
	...
	@Override protected void bindAttributes() {
		...
		super.bindAttribute(3, "boneIDs");
		super.bindAttribute(4, "boneWeights");
	}

	@Override protected void getAllUniformLocations() {
		location_fixPositionMatrix = super.getUniformLocation("fixPositionMatrix");
		...
		location_bindShapeMatrix = super.getUniformLocation("bindShapeMatrix");
		
		location_boneTransforms = new int[MAX_BONES];
		for (int i = 0; i < MAX_BONES; i++) {
			location_boneTransforms[i] = super.getUniformLocation("boneTransforms[" + i + "]");
		}
	}
	...
	public void loadFixPositionMatrix(Matrix4f fixPosMatrix) {
		super.loadMatrix(location_fixPositionMatrix, fixPosMatrix);
	}
	
	public void loadBindShapeMatrix(Matrix4f bindShapeMatrix) {
		super.loadMatrix(location_bindShapeMatrix, bindShapeMatrix);
	}
	
	public void loadBoneMatrices(ArrayList<Matrix4f> boneTransforms) {
		for (int i = 0; i < MAX_BONES; i++) {
			if (i < boneTransforms.size()) {
				super.loadMatrix(location_boneTransforms[i], boneTransforms.get(i));
			} else {
				super.loadMatrix(location_boneTransforms[i], new Matrix4f());
			}
		}
	}
